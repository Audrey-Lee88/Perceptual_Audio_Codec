...
class PACFile(AudioFile):
    ...
    def ReadFileHeader(self):
        ...
        overlapAndAdd = []
        for iCh in range(nChannels): overlapAndAdd.append( np.zeros(nMDCTLines, dtype=float) )
    ...
    def WriteFileHeader(self,codingParams):
        ...
        priorBlock = []
        for iCh in range(codingParams.nChannels):
            priorBlock.append(np.zeros(codingParams.nMDCTLines,dtype=float) )
    ...
    def Close(self,codingParams):
        ...
        if self.fp.mode == "wb":  # we are writing to the PACFile, must be encode
            ...
            data = [ np.zeros(codingParams.nMDCTLines,dtype=float),
                     np.zeros(codingParams.nMDCTLines,dtype=float) ]
...
#    for Direction in ("Decode",):
        ...
        if Direction == "Encode":
            ...
            codingParams.targetBitsPerSample = 2.67
