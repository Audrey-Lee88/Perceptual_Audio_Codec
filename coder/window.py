...
def KBDWindow(dataSampleArray,alpha=4.):
    ...
    window = sp.windows.kaiser_bessel_derived(len(dataSampleArray), alpha)
    return window * dataSampleArray

def HalfSineWindow(N,first=True):
    if first == True:
        window = SineWindow(np.ones(N*2))[0:N]
    else:
        window = SineWindow(np.ones(N*2))[N:2*N]
    return window

def HalfKBDWindow(N, alpha=4., first=True):
    if first == True:
        window = sp.windows.kaiser_bessel_derived(N*2, alpha)[0:N]
    else:
        window = sp.windows.kaiser_bessel_derived(N*2, alpha)[N:2*N]
    return window

def LongtoShortWindow(dataSampleArray, a, b):
    """
    ...
    """
    left = HalfKBDWindow(int(a))
    right = HalfSineWindow(int(b), first=False)
    window = np.concatenate((left, right))

    return window * dataSampleArray

def ShorttoLongWindow(dataSampleArray, a, b, alpha=4.):
    window = np.flip(LongtoShortWindow(dataSampleArray, a, b))
    return window * dataSampleArray
